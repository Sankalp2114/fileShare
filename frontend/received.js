document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchAndDisplayFiles(username, token) {
      ...
      fetch(`http://localhost:3000/recfiles/${username}`, {
        method: "GET",
        headers: {
    ...
    function displayFiles(files) {
        ...
        files.forEach((file) => {
          ...
          sharedToDiv.textContent = file.uploadedBy;
      ...
      function deleteFile(fileId, token,filename) {
       
        fetch(`http://localhost:3000/recdelete/${fileId}`, {
            method: "POST",
            headers: {
